...
- 报告会显示数据完整度评分和主要字段来源
- 对 Yahoo、SEC、新闻和财报表格请求做短期缓存，减少限流
- 可用命令行运行，也可以用网页界面运行
...
每次生成报告时，程序会在 `reports/history/` 里保存一个历史快照。下次再生成同一只股票报告时，会自动比较机会评分、成交额方向估算、分析师共识和关键词次数变化。
...
SEC EDGAR 请求会自动带 `SEC_USER_AGENT`。正式部署前建议设置成真实联系方式：

```bash
export SEC_USER_AGENT="financial-market-analyzer/0.1 your-email@example.com"
```
